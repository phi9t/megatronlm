...
interface CircuitProps {
  ...
  lens: Lens
  selectedId: string
  onSelect: (id: string, block: Block) => void
  lensValue: (id: string) => string
}

export function preludeSelectionId(index: number, block: Block): string {
  return `prelude:${index}:${block.id}`
}

export function headSelectionId(index: number, block: Block): string {
  return `head:${index}:${block.id}`
}

export function layerSelectionId(
  groupIndex: number,
  branchIndex: number,
  branchName: string,
  blockIndex: number,
  ...
  return `layer:${groupIndex}:branch:${branchIndex}:${branchName}:block:${blockIndex}:${block.id}`
}

function branchLabel(name: string): string {
  if (name === 'attn') return 'Attention'
  if (name === 'mlp') return 'MLP'
  if (name === 'moe') return 'MoE'
  return name
}

function layerAriaLabel(group: LayerGroup, branchName: string, block: Block): string {
  return `${group.label ?? 'decoder layer'}, ${branchLabel(branchName)}, ${block.label}`
}
...
export default function ModelCircuit({
  ...
  return (
    ...
    >
      ...
      {groupLayouts.map(({ group, branches }, gi) =>
        branches.map(({ branch, yTee, yJunc }, bi) => (
          <g key={`${gi}-${bi}-${branch.name}`}>
            <BranchPanel
              group={group}
              groupIndex={gi}
              branchIndex={bi}
      ...
      {headYs.map(({ block, y: by }) => (
        <NodeCard
          key={headSelectionId(manifest.head.indexOf(block), block)}
          block={block}
          selectionId={headSelectionId(manifest.head.indexOf(block), block)}
          ...
          h={MAIN_H}
          tag={false}
          selected={selectedId === headSelectionId(manifest.head.indexOf(block), block)}
          onSelect={onSelect}
      ...
      {preludeYs.map(({ block, y: by }) => (
        <NodeCard
          key={preludeSelectionId(manifest.prelude.indexOf(block), block)}
          block={block}
          selectionId={preludeSelectionId(manifest.prelude.indexOf(block), block)}
          ariaLabel={block.label}
          cx={MAINLINE_X}
          cy={by}
          w={MAIN_W}
          h={MAIN_H}
          tag={false}
          selected={selectedId === preludeSelectionId(manifest.prelude.indexOf(block), block)}
          onSelect={onSelect}
          metric={metric(block.id)}
        />
      ))}
...
function BranchPanel({
  ...
  return (
    <g>
      ...
      {cards.map((block, i) => {
        const selectionId = layerSelectionId(groupIndex, branchIndex, name, i, block)
        return (
          <NodeCard
            key={selectionId}
            block={block}
            selectionId={selectionId}
            ariaLabel={layerAriaLabel(group, name, block)}
            cx={CARD_X}
            cy={ys[i]}
            w={CARD_W}
            h={CARD_H}
...
function NodeCard({
  ...
  return (
    <g
      ...
      onClick={() => onSelect(selectionId, block)}
      onKeyDown={(ev) => {
        if (ev.key === 'Enter' || ev.key === ' ') {
          ev.preventDefault()
          onSelect(selectionId, block)
        }
      }}
    >
